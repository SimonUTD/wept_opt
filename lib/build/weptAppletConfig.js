...
const generateWxmlCode = require('../compile/wxml');


const DEFAULT_WX_CONFIG = require('../constantFile/WX_CONFIG');
const DEFAULT_WX_PROJECT_CONFIG = require('../constantFile/WX_PROJECT_CONFIG');

const findImportWxmls = (dir, wxmlList) => {
  const fdList = fs.readdirSync(dir);
  fdList.forEach(fd => {
    const fdPath = path.resolve(dir, fd).replace(/\\/gm, '/');
    const stat = fs.statSync(fdPath);
    if (stat.isFile() && path.extname(fd).toLowerCase() === '.wxml') {
      const fileName = fd.substring(0, fd.length - path.extname(fd).toLowerCase().length);
      if (!fdList.includes(`${fileName}.json`)) {
        wxmlList.push(fdPath);
      }
    } else if (stat.isDirectory()) {
      findImportWxmls(fdPath, wxmlList);
    }
  });
};

const buildWxmlImport = (absoluteProjectPath) => {
  let wxmlList = [];
  findImportWxmls(absoluteProjectPath, wxmlList);
  return wxmlList.map(file => `./${file.substring(absoluteProjectPath.length + 1)}`);
};
...
const normalizeLink = (absoluteProjectPath, file) => {
  return path.relative(absoluteProjectPath, file).replace(/\\/gm, '/');
};

const sortAppFirst = (a, b) => {
  if (a === 'app.js') return -1;
  if (b === 'app.js') return 1;
  return a.localeCompare(b);
};

const buildJsLinks = (absoluteProjectPath, dist) => {
  const ignore = [
    path.resolve(absoluteProjectPath, '**', 'node_modules', '**'),
    path.resolve(absoluteProjectPath, '**', '.git', '**'),
    path.resolve(absoluteProjectPath, '**', '.DS_Store')
  ];
  if (dist) {
    ignore.push(path.resolve(dist, '**'));
  }
  return glob
    .sync(path.resolve(absoluteProjectPath, '**', '*.js'), { ignore, nodir: true })
    .map(file => normalizeLink(absoluteProjectPath, file))
    .sort(sortAppFirst);
};
...
class WeptAppletConfig {
  constructor (options = {}) {
    ...
    this.wxmlImportFiles = buildWxmlImport(this.appletSources);
    this.links = buildJsLinks(this.appletSources, this.compiledPath);
  ...
  getWxmlCode (isService) {
    return generateWxmlCode(this.appletSources, this.wxConfig, this.wxmlImportFiles, this.wxsList, this.components,isService);
  }
