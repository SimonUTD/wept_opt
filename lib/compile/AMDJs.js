...
const { createBabelOptions } = require('../babelOptions');

const _compileJS = (projectConfig, sourceFile, targetFile, moduleName) => {
  logs.info(`转换文件： ${moduleName}`);
  ...
  if (projectConfig.setting.es6 && fs.statSync(sourceFile).size / 1024 < 500) {
    const options = createBabelOptions();
    content += babel.transformFileSync(sourceFile, options).code;
  } else {
...
module.exports = compileAMDModules;
